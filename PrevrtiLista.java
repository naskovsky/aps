...
public class PrevrtiLista {
    ...
    public static boolean parna(DLLNode<Integer> broj) {
        if (broj.element % 2 == 0) {
            return true;
        } else {
            return false;
    ...
    public static void main(String[] args) throws IOException {
        ...
        DLLNode<Integer> marker = lista.getFirst();
        if (lista.length() == 1 || lista.length() == 0) {
            System.out.println(lista);
        }
        while (node.element != null) {
            if (parna(node) && !parna(node.succ)) {
                marker = node;
                break;
            }
            node = node.succ;
        }
        DLLNode<Integer> pointer = marker;

        while (pointer != null) {
            pomosna.insertLast(pointer.element);
            pointer = pointer.pred;
        }
        while (last != marker) {
            pomosna.insertLast(last.element);
            last = last.pred;
        }
